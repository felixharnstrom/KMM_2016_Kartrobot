...
\begin{description}
    ...
    \item [KEY\_EVENT] Om vi är i det manuella läget: uppdatera motorstyrning med skickad tangent och dess tillstånd (nedtryckt eller släppt). Tangenten som skickas därefter är en sträng som innehåller ''\texttt{left}'', ''\texttt{right}'', ''\texttt{up}'', eller ''\texttt{down}'' med suffixet ''\texttt{\_p}'' om tangenten tryckts ner eller suffixet ''\texttt{\_r}'' om tangenten släppts.
    ...
    \item [SYNC\_MODE] Hämtar vilket läge som roboten i nuläget står i.
\end{description}
...
För att styra vilken vinkel som servot står i så används en PWM-signal. Denna signal bestäms genom att omvandla den önskade vinkeln till ett värde som sätter en duty-cycle hos PWM-signalen. Värdet beräknas enligt \ref{eq:servo_degree_to_pwm}.
\begin{equation}\label{eq:servo_degree_to_pwm}
value =  773 + angle \times 8.72
\end{equation}
Detta värde bestämmer hur många steg i en räknare som PWM-signalen kommer att vara ettställd, med andra ord hur lång som duty-cyclen är.
%WIP
  Räknaren i sig räknar från $0 -- 20000$ med uppräkningsfrekvensen 
  Detta värde som måste sättas utgår från att PWM-signalen ligger på 50Hz, och att en duty-cycle på $3.9-11.7\%$ täcker det vinkelspann som servot kan röra sig i, nämligen 0-180\si{\degree}.  
