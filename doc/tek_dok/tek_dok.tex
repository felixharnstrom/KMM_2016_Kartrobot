...
\clearpage
\section{Produkten}
%TODO : Bild med förklarande text.
...
\subsubsection{Reflex sensorer} \label{ssec:reflex}
Reflex sensorerna skickar ut en analog signal som varierar beroende på hur mycket ljus som avges från ytan den siktar på. Eftersom att hjulet har en färgskala påklistrad så kommer signalen att variera i värde för varje gång som hjulet snurrat 1/8 av ett varv.

\subsubsection{LP-filter} \label{sssec:lp-filter}
De sensorer som skickar en analog signal kommer att få med sig endel brus, vilket kan medföra att mätvärdena får stora avvikelser. För att få bort detta brus så går sensorer med analoga signaler (förrutom reflex sensorer) till LP-filter som filtrerar bort frekvenser över ungefär 88 Hz. Referenssignalen som AD-omvandlarna använder har också ett LP-filter då det förbättrar noggranheten i avläsningen betydligt.

\begin{figure}[h!]
	\makebox[\textwidth][c]{\includegraphics[width=0.5\textwidth]{lp-filter.png}}
...
\hardware{Resistor}{Till LP-filter. \SI{18}{\kilo\ohm}}{5}
...
Reflex sensorerna skickar en analog signal till AD-omvandlarna på ATmega:n. Eftersom att spänningen skljer sig ganska mycket beroende på om reflex sensorn pekar på en svart eller vit yta så behöver vi inget LP-filter här.
