...
Styrenheten lyssnar på UARTen efter instruktioner, utformade enligt specifikationen i \ref{ssec:seriellkommunikation}. Dessa instruktioner tas emot och avkodas till de funktionsanrop som de representerar, och exekveras sedan med de parametrar som hör funktionen till. Därefter återgår enheten till att lyssna efter nya instruktioner.
...
%TODO: Mer ingående detaljbeskrivning av hur PWM-signalen bestäms på styrenheten
...
Delsystemet är vad som i slutändan kontrollerar de olika delsystemen, och kan därför skicka meddelanden mellan dessa. Mjukvara för att kunna skicka meddelanden över de olika gränssnitten som den kopplas upp emot  finns (se kapitel \ref{ssec:brainInterface}).

Förutom hårdvarugränssnitt kan den också kommunicera med en extern PC (se kapitel \ref{sec:system4}) med hjälp av Wi-Fi-kommunikation, där den skickar debugdata och kartinformation samt tar emot meddelanden. Dessa meddelanden behandlas i den ordning de köats i pc-mjukvaran och används för att antingen byta läge på roboten, eller för att ge kommandon under manuell styrning. Mjukvaran för Wi-Fi-kommunikationen ligger som en egen tråd för att utgöra mjukvarukomponenten i kommunikationsmodulen. För en illustration över hur Wi-Fi-kommunikationen fungerar, se figur \ref{fig:wifi}.
...
För att hantera meddelanden mellan de interna såväl som de externa modulerna så används köer. Element läggs till i dessa under avbrottsrutiner, men även direkt under mjukvarukörning på olika trådar (Se figur \ref{fig:wifi} som exempel). 
...
\noindent
Kod finns för var och en av dessa rutiner, men i ett makroperspektiv blir inte mjukvaran särskilt komplex. Se figur \ref{fig:manual_flowchart} för ett övergripande flödesschema.
...
När roboten hitta tillbaka till en yttervägg kommer denne återigen att köra tills den hamnat i garage-rutan.
...
Detta delsystem kommunicerar med Kommunikations- och kontrollenheten (se kapitel \ref{sec:system3}) för att ta emot diagnostisk data, den upptäckta kartan, byta läge på roboten,  samt för att skicka tangentevent som används för att styra roboten i det manuella läget. Se figur \ref{fig:unitPC} för ett övergripande blockschema över PC-hårdvaran.
...
\subsection{Mjukvara}
PC-mjukvaran ansvarar för att kommunicera med roboten över Wi-Fi, låta användaren skicka tangentevent till roboten, och visa diagnostik från roboten såväl som kartan. Se figur \ref{fig:gui_flowchart} för ett övergripande flödeschema för PC-mjukvaran. Koden är skriven i Python 3, och följer \cite{pep8}. Den består av två trådar där den ena hanterar GUIt och den andra hanterar kommunikation med roboten via en socket och med GUIt via köer.
...
PC:n kommunicerar med roboten över Wi-Fi. I mjukvarulagret används Pythons inbyggda sockets. För en ingående beskrivning över protokoll och kommandon, se \ref{ssec:wificommunication}.
