...
\subsection{Seriell kommunikation}\label{sec:serial}
...
\subsection{WIFI-kommunikation}\label{sec:wifi}
...
\subsection{Mjukvara}
Koden skrivs i C, och ska följa standarden specificerad i bilaga \ref{app:cstandard}. 
...
Roboten konstrueras med två olika lägen: ett manuellt, och ett autonomt. När roboten startar så initieras den med startvärden och hamnar i den huvudloop som ser till att den alltid är i det styrläge som användaren avser, se figur \ref{fig:lageskontroll}.
...
\end{figure}
\subsection{Hårdvara}
PC-mjukvaran är körbar på en godtycklig PC med Python3 och Wi-Fi-modul. Lämpligen med skärm, tangentbord, och datormus för kommunikation med användaren.
...
PC-mjukvaran är implementerad som en klient som ansluter till serverprocessen som körs på kommunikations- och kontrollenheten (se kapitel \ref{ssec:brain_comm}). För att ansluta till roboten krävs därför endast serverns IP-adress. Se kapitel \ref{sec:wifi} för en mer ingående beskrivning av hur denna erhålls.
