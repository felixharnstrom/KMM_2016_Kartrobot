...
Det placeras två stycken IR-sensorer av modell GP2D120 på både höger och vänster sida om roboten (för totalt fyra), samt en på baksidan. Det behövs inte någon IR-sensor frammåt för LIDAR kommer att användas till det ändamålet. Dessa sensorer används för navigering och positionsuppskattning. Med hjälp av de dubbla IR-sensorerna på vardera sida kan det avgöras ifall roboten åker parallelt med väggen eller inte. Dessa behöver en A/D-omvanlare var, som är inbyggd i processorn ATmega1284. Se kapitel \ref{ssec:sensorInterface}. Sensorerna kan effektivt mäta avstånd inom intervallet 4 till 30 cm, vilket räcker, då roboten är tänkt att följa en vägg. Rutnätet på banan är nämligen 40 gånger 40 cm.
...
LIDAR används också som en sensor framåt, för attt avgöra ifall robotten kan fortsätta att köra.
...
\subsubsection{LIDAR}
LIDAR kommer i första hand använda avläsningstriggers och PWM via en gemensam pin. PWM-signalen kopplas i så fall till ett avbrott för att mäta dess längd, som används för att beräkna avståndet.

Under körning kommer mätdatan från LIDAR att användas för att avgöra om robotten kan fortsätta framåt.
...
IR-sensorerna använder en analog utgång som varierar i enlighet med avståndet efter en ickelinjär kurva. Denna signal har störningar som måste filtreras ut, med flera mätningar och mjukvara.
