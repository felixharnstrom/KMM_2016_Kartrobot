...
<<<<<<< 5f72b6d0f5aef6fbab97390cd7d2ccf20f977ef1
...
Som vi ser i tillstånds schemat för styrenheten \ref{fig:stateDiagram} så har vi totalt 3 olika tillstånd. Medans styrenheten är i tillståndet WAIT så kommer roboten att stå stilla och invänta på en instruktion på hur den ska köra. Notera att en instruktion kan sättas i alla tillstånd, så om roboten redan har en instruktion inläst då tillståndet sätts till WAIT så kommer roboten börja på denna instruktion omedelbart.
...
Eftersom att hårdvaran sköter inläsning från en pinne parallellt så ska vi inte behöva fler än 1 tråd som sköter logiken för styrenheten.

\subsection{Reglering}
Eftersom att styrenheten inte har tillgång till data i sensorenheten så blir det kontrollenheten som ansvarar för roboten kör rakt. Styrsignalerna kommer därför vara ganska korta intervall så att roboten hinner reglera sin position innan den kör in i en vägg. 
...
\begin{itemize}
	...
	\item Hur långt roboten ska köra i den riktningen. Denna information är endast intressan om instruktionen är av typen DRIVE.
