...
Detta delsystem kommunicerar med Kommunikations- och kontrollenheten (se \ref{sec:system3}) för att ta emot diagnostisk data, den upptäckta kartan, samt ge instruktioner vid manuellt läge. Se figur \ref{fig:unitPC} för ett övergripande blockschema över PC-mjukvaran.
...
\end{figure}
\subsection{Hårdvara}
PC-mjukvaran ska vara körbar på en godtycklig PC med Python 3 och blåtandsmodul.

\subsubsection{Komponentbudget}
Här följer en lista på all hårdvara som detta delsystem kräver.

\begin{itemize}
\item 1 st Bärbar PC med blåtandsmodul
\end{itemize}

\subsection{Mjukvara}
PC-mjukvaran ska ansvara för att kommunicera med roboten över blåtand och låta användaren skicka kommandon till roboten, och visa diagnostik från roboten såväl som kartan. Se bilaga \ref{app:flowchart} för ett övergripande flödeschema för PC-mjukvaran. Koden ska vara skriven i Python 3, och ska följa \cite{pep8}.
...
Det grafiska gränssnittet ska byggas med biblioteket \textit{Tkinter}.
...
PC:n kommunicerar med roboten över blåtand. I mjukvarulagret används Pythonmodulen \textit{PyBluez}.\footnote{\url{http://karulis.github.io/pybluez/}} 
