...
\begin{LIPSdokumenthistorik}
    \LIPSversionsinfo{1.1}{2016-11-13}{Wi-Fi istället för blåtand}{FH}{}
    \LIPSversionsinfo{1.0}{2016-11-03}{Upphöjt till version 1.0}{FH, SL}{FH}
...
För kommunikation mellan Kommunikation- och kontrollenheten (se kapitel \ref{sec:system3}) och sensorenheten samt styrenheten (se kapitel \ref{sec:system2} respektive \ref{sec:system3}) används seriella bussar. Då Raspberry pi:n som används i kontrollenheten saknar inbyggd UART så används USB-till-UART-moduler. Vidare så kommunicerar Kommunikation- och kontrollenheten via Wi-Fi med PC-mjukvaran (se kapitel \ref{sec:system4}).
...
Feedback fås i den senare delen av projektet via PC-mjukvaran (se kapitel \ref{sec:system4}). Tidigare så kan LED-skärmar och oscilloskop användas för debugging.
...
\hardware{EXO-3}{Oscillator för ATmega-klockan, med standardfrekvens \SI{16}{\mega\hertz}}{1}
...
\item Hobbyservot styrs med PWM vilket är betydligt enklare att implementera än half-duplex. Svängvidden för hobbyservon brukar vara runt 180 grader. Vi använder dock gärna ett med större svängvidd för att behöva skanna färre gånger.
\end{itemize}
...
Kommunikations- och kontrollenheten använder sig av seriella bussar för datautbyte mellan sensorenhet och styrenhet. Mjukvara för dessa krävs på båda håll. Utbyte av data måste kunna ske åt båda hållen; sensorenheten skickar data på kommando, och styrenheten skickar avläsningsdata.

Utöver detta behövs fungerande Wi-Fi-kommunikation. Raspberry PI:ns inbyggda Wi-Fi-modul används, och vi förväntar oss att PC:n som har användargränssnittet (se kapitel \ref{sec:system4}) även har support för denna typ av kommunikation. I mjukvarulagret används Pythonmodulen \cite{pybluez}.
...
\begin{center}
\begin{HardwareList}
\hardware{PC}{Bärbar PC med Wi-Fi-modul.}{1}
\end{HardwareList}
\end{center}
...
PC:n kommunicerar med roboten över Wi-Fi. I mjukvarulagret används Pythonmodulen \cite{pybluez}.
