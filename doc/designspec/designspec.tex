...
\section{Sensorenhet}
Sensorenheten har i uppgift att läsa in sensordata och omvandla den till ett läsligt format. Den omsveper en processor av modellen ATmega1284, flera IR-sensorer, en lasersensor, samt gyro.

IR-sensorerna används primärt för navigering, så att roboten har lättare att köra i en rak linje längs en vägg. Lasersensorn används för skanning av rummet. Med hjälp av gyrot är det lättare att hålla koll på robotens rotation.
...
ATmega1284p används som processormodell, då den är kraftfull utan att ta det till överdrift.

\subsubsection{IR-sensorer} \label{sssec:sonicsensors}
Det placeras två stycken IR-sensorer av modell GP2D120 på både höger och vänster sida om roboten (för totalt fyra), samt en på baksidan. Dessa sensorer används för navigering och positionsuppskattning. Med hjälp av de dubbla IR-sensorerna på vardera sida kan det avgöras ifall roboten åker parallelt med väggen eller inte. Dessa behöver en A/D-omvanlare var. Se kapitel \ref{ssec:sensorInterface}. Sensorerna kan effektivt mäta avstånd inom intervallet 4 till 30 cm, vilket räcker, då roboten är tänkt att följa en vägg. Rutnätet på banan är nämligen 40 gånger 40 cm.

\subsubsection{LIDAR lite v2} \label{sssec:lidar}
LIDAR är en kraftfull lasersensor som används för mätningar som kräver bättre noggranhet. Komponenten kommuniceras med via en trigger-pin och PWM-output.

Sensorn monteras på toppen av roboten - ovanpå ett roterande servo, som specificeras i större detalj i kapitel \ref{ssec:servomotor}. Detta för att effektivt kunna mäta avstånd i flera vinklar.
...
%\begin{HardwareList}
%\hardware{Atmega1284p}{Mikroprocessor med 44 pinnar.}{1}
%\hardware{GP2D120}{IR-sensor.}{5}
%\hardware{AD-omvandlare}{För vardera av IR-sensorerna.}{5}
...
Programmet omvandlar sensordata till ett mer läsligt format, och skickar det vidare (se kapitel \ref{ssec:sensorInterface}), samt tar emot instruktioner om att läsa av en viss sensor. % TODO Avläsningsinstruktioner? Menas kommandon som: ``Tja, jag ser gärna detta avläst''? Bör förtydligas och utvecklas. - Tillräckligt?
...
Sensorenheten ska kommunicera med de sensorerna och passera vidare utdata. Sensorenheten tar emot uppmaningar om att starta en avläsning via samma buss som används för att rapportera mätvärden vidare.
...
\subsubsection{LIDAR}
LIDAR kommer i första hand använda avläsningstriggers och PWM via en gemensam pin. PWM-signalen kopplas i så fall till ett avbrott för att mäta dess längd, som används för att beräkna avståndet. Som alternativ ifall behov uppstår kan också I\textsuperscript{2}C användas.

\subsubsection{Gyro}
MLX90609 stödjer avläsning via en analog pinne samt via SPI. Vi använder analog avläsning. På kortet finns också en tempeartursensor för att att man ska kunna kompensera för störningar av temperaturskillnader. Denna implementeras endast om störningarna blir oväntat stora.

\subsubsection{IR-sensorer}
IR-sensorerna använder en analog utgång som varierar i enlighet med avståndet efter en ickelinjär kurva. Denna signal har störningar som måste filtreras ut, antingen med ett LP-filter eller med flera mätningar och mjukvara.
