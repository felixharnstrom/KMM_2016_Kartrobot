...
Det placeras två stycken IR-sensorer av modell GP2D120 på vardera sida om robotton, samt en på baksidan. Dessa sensorer används för navigering och positionsuppskattning. Med hjälp av dem dubla R-sensorerna per sida kan makt avgöras ifall roboten åker paralelt med väggen eller inte. Dessa behöver en utgång och en interrupt-ingång var. Se kapitel \ref{ssec:sensorInterface}. Sensorerna kan mäta avstånd inom intervallet 4-30cm, vilket räcker för att roboten är tänkt att följa en vägg. För att rutnätet på banan är 40 gånger 40 cm så räcker den smala avsläsningsintervallet.
...
MLX90609 är ett gyro, som används via en analog ingång. Den ger oss endast rotationen kring z-axeln, som användas för att beräkna robotens riktning i rummet.
