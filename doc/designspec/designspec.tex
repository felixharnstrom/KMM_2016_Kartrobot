...
%TODO Update this figure to use IR sensors and gyro
\begin{figure}[h!]
...
ATmega1284 används som processormodell, då den är kraftfull utan att ta det till överdrift.

\subsubsection{IR-sensorer} \label{sssec:sonicsensors}
Det placeras två stycken IR-sensorer av modell GP2D120 på vardera sida om robotton, samt en på baksidan. Dessa sensorer används för navigering och positionsuppskattning. Med hjälp av dem dubla IR-sensorerna per sida kan det avgöras ifall roboten åker paralelt med väggen eller inte. Dessa behöver en A/D-omvanlare var. Se kapitel \ref{ssec:sensorInterface}. Sensorerna kan mäta avstånd inom intervallet 4-30cm, vilket räcker för att roboten är tänkt att följa en vägg. För att rutnätet på banan är 40 gånger 40 cm så räcker den smala avsläsningsintervallet.


\subsubsection{LIDAR lite v2} \label{sssec:lidar}
LIDAR är en kraftfull lasersensor som används för mätningar som kräver noggranhet. Komponenten kan kommuniceras med via en trigger-pin och PWM-output, alternativt via en I\textsuperscript{2}C-buss. I första hand kommer PWM användas.
...
MLX90609 är ett gyro som används via en analog ingång. Den ger oss endast rotationen kring z-axeln, som användas för att beräkna robotens riktning i rummet under svängar.
...
Programmet ska omvandla sensordata till ett mer läsligt format, och ska kunna skicka det vidare (se \ref{ssec:sensorInterface}), samt ta emot avläsningsinstruktioner.
